...
title_zh: 从记忆中协调：面向动态制造中多智能体自适应的图结构经验复用
...
score: 7.0
evidence: 多智能体强化学习中的经验重用进行协调
tldr: "动态制造环境中的多智能体系统需应对机器故障、紧急订单等频繁干扰。现有方法丢弃协调经验，导致适应缓慢。本文提出图结构经验记忆框架，将历史协调编码为异质关系图，利用图神经网络检索相似经验指导策略调整。在动态柔性作业车间调度中，该方法相比最强基线降低4.1%-10.0%的完工时间与33%-38%的适应时间，且优势随干扰频率增加而扩大。消融实验和跨干扰迁移验证了图编码与相似性检索的必要性及协调模式的泛化性。"
source: arxiv
...
motivation: 现有方法丢弃历史协调经验，每次干扰需从头学习，导致适应效率低且难以应对动态扰动。
method: 提出图结构经验记忆（GSEM）框架，将历史协调编码为异质关系图，用图神经网络检索相似经验以指导策略适应。
result: "在动态柔性作业车间调度中，GSEM相比最强记忆增强基线降低4.1%-10.0%的完工时间与33%-38%的适应时间。"
conclusion: 图结构记忆能有效加速多智能体对动态干扰的适应，且学习到的协调模式可跨干扰泛化。
---

## 摘要
动态制造环境要求多智能体系统在频繁的运作扰动（如机器故障、紧急订单到达和加工时间变化）下有效协调。现有的大多数多智能体强化学习方法独立处理每次扰动事件，丢弃了可能加速未来适应的宝贵协调经验。本文提出了一种面向动态制造中多智能体协调的图结构经验记忆（GSEM）框架。该框架将历史协调经验编码为异质关系图，捕获任务依赖关系、机器状态以及智能体间协作模式。当新的扰动发生时，基于图神经网络的检索机制识别结构相似的历史经历，从而实现经验引导的策略自适应，而非从零学习。在包含三种扰动类型的动态柔性作业车间调度基准上的实验表明，与最强的记忆增强基线相比，GSEM将完工时间减少了4.1%-10.0%，自适应时间减少了33%-38%，且在高扰动频率下优势更加明显。消融研究和跨扰动迁移实验进一步验证了图结构编码和基于相似性检索的必要性，并展示了所学协调模式的跨扰动泛化能力。

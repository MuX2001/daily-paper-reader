...
tags: ["query:control"]
score: 7.0
evidence: 基于谱图替代目标实现移动智能体网络控制
tldr: 异构移动代理网络中，通信代理需随任务代理运动实时重定位。直接吞吐量优化不适合实时控制，常规采用代数连通性作为替代目标，但其控制依赖Fiedler向量，分布式估计通信开销大。本文揭示Fiedler梯度控制器可分解为局部交互与图嵌入结构，据此提出以邻接矩阵主特征向量替代Fiedler向量的A-Fiedler方法。实验表明，无通信约束时性能与经典方法相当，且在分布式估计下鲁棒性更好，为实时分布式网络控制提供了更简单有效的途径。
source: arxiv
...
motivation: 实时控制移动通信代理困难，代数连通性虽为常用替代指标，但其控制器依赖Fiedler向量，分布式估计需大量通信。
method: 将Fiedler梯度控制器分解为局部交互与图嵌入组件，提出以邻接矩阵主特征向量作为替代嵌入的A-Fiedler方法。
result: 无通信约束时性能与经典Fiedler梯度控制器相当；分布式估计下鲁棒性更好，相同通信轮数下不会像后者一样收敛到断开配置。
conclusion: 提供更简单有效、适合局部的分布式网络控制途径，有潜力推广到实时移动代理系统。
---

## 摘要
我们考虑一个由不受控的任务智能体和可控的通信智能体组成的异构移动智能体网络。目标是在任务智能体移动时在线 reposition 通信智能体。由于基于吞吐量的目标通常不适合实时控制，谱图指标（如代数连通度）常被用作替代目标。然而，控制代数连通度依赖于图的拉普拉斯矩阵第二小特征值对应的特征向量（即 Fiedler 向量），其分布式估计需要无界轮数的通信才能收敛。在本工作中，我们识别出该 Fiedler 梯度控制器的一种结构分解，将其分解为局部交互规则和图嵌入组件，这表明可以使用比 Fiedler 向量更易于分布式估计的替代嵌入。作为一个具体实例，我们提出了 A-Fiedler，它将 Fiedler 嵌入替换为邻接矩阵的主特征向量，该向量通常用作节点到潜在几何空间的图嵌入。这种表示在局部通信约束下更适合分布式实现。我们将 A-Fiedler 与经典 Fiedler 梯度控制器进行了评估。结果表明，在无通信约束时网络性能相当，而在分布式估计下鲁棒性更好。例如，在相同轮数的通信下，Fiedler 梯度甚至可能收敛到不连通的配置，而我们的方案保持了性能。我们相信我们的贡献为分布式网络控制提供了一条更简单的路径。

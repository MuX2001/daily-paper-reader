...
tags: ["query:vla"]
score: 7.0
evidence: 面向机器人系统VLA模型部署的端边协同推理优化
tldr: "具身智能中VLA模型推理成本高，设备端算力与能耗受限，边缘卸载又受网络波动影响，难以兼顾实时控制与能效。EcoVLA通过统一阶段级抽象与延迟能耗预测，自适应选择满足实时约束的能效最优协同方案，并优化中间张量传输。在20Hz动作输出频率下，系统能效较现有协同方法最高提升236%，动态网络与边缘负载下仍满足SLO。为VLA模型实时节能部署提供了系统化解决方案。"
source: arxiv
...
motivation: 设备端推理受算力能耗制约，边缘卸载面临延迟波动，现有协同推理缺乏针对VLA模型的统一实时能效方案。
method: EcoVLA提出统一阶段级抽象与延迟能耗预测，运行时毫秒级选择能效最优方案，并优化中间张量传输。
result: "在20Hz动作输出约束下，系统能效较现有协同方法最高提升236%，动态网络与边缘负载下仍满足SLO。"
conclusion: EcoVLA为VLA模型提供架构无关的实时节能协同推理框架，显著提升能效并保障服务质量，助力具身智能落地。
---

## 摘要
视觉-语言-动作（VLA）模型已成为具身智能的一种有前景的基础，但其高昂的推理成本为在机器人系统中的部署带来了重大挑战。在实践中，设备端推理受限于有限的计算能力和能耗预算，难以同时满足实时控制与能效要求。或者，将推理负载卸载到边缘服务器容易受到系统条件波动的影响，带来不可预测的延迟风险。设备-边缘协同推理提供了一种有前景的解决方案，但针对VLA模型的系统性研究仍然匮乏，尤其是缺乏一个统一处理实时约束和系统级能效的协同推理框架。为此，我们提出EcoVLA，一种面向VLA模型的自适应设备-边缘协同推理框架，在实时约束下最大化系统能效。EcoVLA首先对不同VLA范式引入统一的阶段级抽象，建立与架构无关的协同推理设计空间。然后，它构建了一个联合的设备-边缘-网络延迟与能耗预测模型，以实现候选协同推理方案的快速运行时评估。在此基础上，EcoVLA持续选择满足实时约束且开销为毫秒级的能效最优方案，并适应网络和系统状态的运行时变化。此外，EcoVLA包含一种用于阶段间中间张量的轻量级传输机制，以减少跨设备协作带来的通信开销。跨VLA模型的实验结果表明，在20 Hz动作输出频率约束下，EcoVLA相比现有协同推理方法将系统能效提升高达236%，同时在动态网络和边缘工作负载条件下持续满足SLO要求。

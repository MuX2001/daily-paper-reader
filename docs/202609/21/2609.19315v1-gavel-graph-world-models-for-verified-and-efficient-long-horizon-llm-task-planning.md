...
score: 8.0
evidence: 用于长时程机器人规划的显式图世界模型，可预测动作后果
tldr: 大语言模型为长时程机器人规划提供灵活接口，但生成的计划常违反本体约束、难以从错误中恢复且部分可观测下推理薄弱。本文提出GAVEL，以显式图世界模型表示物体关系、动作前置条件与效果以及未观测物体的概率信念，在执行前预测动作后果、检测违规并修复计划，仅在必要时调用LLM重规划。该方法提升了长时程规划的可靠性与效率。
source: arxiv
...
motivation: 用于长时程机器人规划的显式图世界模型，可预测动作后果。

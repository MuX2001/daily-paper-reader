...
tags: ["query:vla"]
score: 8.0
evidence: 面向长时程任务编排训练部署VLA智能体的统一框架
tldr: "VLA模型难以独立胜任长时程任务。EmbodiedSkills把技能决策当作执行提议，执行前检查前提，执行后验证结果，用固定接口整合高层选择与低层执行。在RoboTwin 2.0和LIBERO上，任务适应VLA策略平均成功率分别达86.20%和97.40%，RMBench上仅12.5%。该框架提供可训练、可检查的智能体层。"
source: arxiv
...
motivation: VLA动作预测难以独立完成长时程任务，技能决策缺乏执行前提检查与结果验证。
method: 将技能决策作为执行提议，运行时检查前提并验证结果，通过固定接口整合高层技能选择与低层VLA执行。
result: "任务适应低层策略在RoboTwin 2.0/LIBERO/RMBench上平均成功率分别为86.20%、97.40%和12.5%。"
conclusion: 该框架提供可训练、可检查的智能体层，固定接口允许替换低层策略，实现闭环具身系统。
---

## 摘要
视觉-语言-动作（VLA）模型将视觉观察和语言指令直接映射到机器人动作，但长时程任务需要的不仅仅是动作预测。代理必须在物理状态演化过程中协调感知、规划、执行、进度验证和恢复。动作预测或模型生成的技能决策本身并不能保证所提议的操作在当前状态下有效，也不能保证其结果将得到验证。我们提出EmbodiedSkills，一个统一框架，将每个技能决策视为执行提议：运行时在执行前检查其前提条件，并在执行后验证结果。共享的可执行技能接口在单个代理循环中连接了高层技能选择、受限的低层VLA执行和动作后验证。由于该接口保持固定，低层VLA策略可以被替换或调整，而无需更改代理循环。该接口还将规划、执行、验证和恢复事件记录为结构化轨迹，为各个组件提供监督，并在交互反馈可用时支持可选的在线适应。我们使用Qwen3-VL和OpenPI/pi0.5在RoboTwin 2.0和LIBERO上实例化了EmbodiedSkills。任务适应的低层VLA策略在50个RoboTwin 2.0任务中平均成功率达到86.20%，在四个LIBERO套件中平均成功率达到97.40%。这些结果确立了EmbodiedSkills中使用的任务适应低层VLA策略的执行性能。在四个依赖记忆的RMBench任务上，相同的任务适应执行方法实现了12.5%的平均成功率。该框架提供了一个可训练和可检查的代理层，用于将这些策略转化为闭环具身系统。
